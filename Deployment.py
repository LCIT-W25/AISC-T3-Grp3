...
if st.button("Analyze Sentiment"):
    if review_text:
        ...
        if use_nb:
            ...
            st.write(f"**Naïve Bayes Prediction ({nb_version}):** {'Positive' if nb_prediction else 'Negative'}")

        if use_svm:
            svm_prediction = svm_model.predict(transformed_text)[0]
            st.write(f"**SVM Prediction ({svm_version}):** {'Positive' if svm_prediction else 'Negative'}")
    else:
...
def extract_resnet_features(image):
    ...
    tensor = transform(image).unsqueeze(0).to(device)  # Apply transformations
    
    with torch.no_grad():
        features = feature_extractor(tensor)  # Shape: [1, 512, 1, 1]

    flattened_features = features.view(-1).cpu().numpy()

    print(f"Extracted feature shape: {flattened_features.shape}") 
    return flattened_features  # Flatten to [512]
